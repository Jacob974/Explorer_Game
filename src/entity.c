...
void detectCollisionPoint(Entity* entity, TileMap* tileMap)
{
    ...
    for(int i = 0; i < tileMap->tileHight * tileMap->tileWidth; i++) 
    {
        if(tileMap->tileList[i].exists == 1)
        {
            if(entity->accel.x < 0)
            {
                xNearCollisionPoint = tileMap->tileList[i].coords.x * 32 + 32 + (entity->dest.w / 2); //half the entity's length is subtracted rounded down to compensate for the collion detection happening in the center of the entity
                xFarCollisionPoint = tileMap->tileList[i].coords.x * 32 - entity->dest.w + (entity->dest.w / 2); //half the entity's length is added rounded up to compensate for the collion detection happening in the center of the entity
            }
            else
            {
                xNearCollisionPoint = tileMap->tileList[i].coords.x * 32 - entity->dest.w + (entity->dest.w / 2); //half the entity's length is added rounded up to compensate for the collion detection happening in the center of the entity
                xFarCollisionPoint = tileMap->tileList[i].coords.x * 32 + 32 + (entity->dest.w / 2); //half the entity's length is subtracted rounded down to compensate for the collion detection happening in the center of the entity
            }
            if(entity->accel.y < 0)
            {
                yNearCollisionPoint = tileMap->tileList[i].coords.y * 32 + 32 + (entity->dest.h / 2); //half the entity's length is subtracted rounded down to compensate for the collion detection happening in the center of the entity
                yFarCollisionPoint = tileMap->tileList[i].coords.y * 32 - entity->dest.h + (entity->dest.h / 2); //half the entity's length is added rounded up to compensate for the collion detection happening in the center of the entity
            }
            else
            {
                yNearCollisionPoint = tileMap->tileList[i].coords.y * 32 - entity->dest.h + (entity->dest.h / 2); //half the entity's length is added rounded up to compensate for the collion detection happening in the center of the entity
                yFarCollisionPoint = tileMap->tileList[i].coords.y * 32 + 32 + (entity->dest.h / 2); //half the entity's length is subtracted rounded down to compensate for the collion detection happening in the center of the entity
            }
